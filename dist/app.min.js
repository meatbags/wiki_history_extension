...
class View {
  constructor() {
    ...
    this.el.graphMonthly.classList.add('active'); // storage
    ...
    this.el.graphMonthly.appendChild(this.createColumn(monthStamp)); // ui

    this.el.graphSlider.addEventListener('mousedown', e => {
      this.onMouseDown(e);
    });
    this.el.graphSlider.addEventListener('mousemove', e => {
      this.onMouseMove(e);
    });
    this.el.graphSlider.addEventListener('mouseup', e => {
      this.onMouseUp(e);
    });
    this.el.graphSlider.addEventListener('mouseleave', e => {
      this.onMouseLeave(e);
    });
  ...
  addGraphData(graph, timestamp, data) {
    const el = graph.querySelector(`[data-timestamp="${timestamp}"]`); // check for unique user
  ...
  addMissingColumns(timestamp, type) {
    const graph = type == 'week' ? this.el.graphWeekly : this.el.graphMonthly; // get week/ month start date
    ...
    const b = new Date(lastChild.dataset.timestamp).getTime(); // add missing columns

    if (a < b) {
      const dates = type == 'week' ? this.time.getWeeks(stamp, lastChild.dataset.timestamp) : this.time.getMonths(stamp, lastChild.dataset.timestamp); // add columns if they don't exist
  ...
  addRevisions(revs) {
    ...
    for (let i = index, len = this.dataset.length; i < len; ++i) {
      ...
      const monthStamp = this.time.getMonthStart(item.timestamp).toISOString(); // add missing elements
  ...
  onMouseMove(e) {
    ...
    this.mouse.y = e.clientY; // calculate mouse delta and pan graph
    ...
    if (e.target && (e.target.classList.contains('column__inner') || e.target.classList.contains('column'))) {
      ...
      const dateString = this.el.graphMonthly.classList.contains('active') ? `${months[date.getUTCMonth()]} ${date.getFullYear()}` : `Week of ${date.getUTCDate()} ${months[date.getUTCMonth()]} ${date.getFullYear()}`;
