...
class View {
  constructor() {
    ...
    this.el.graphSlider.addEventListener('mouseleave', e => { this.onMouseUp(e); });
  ...
  incrementColumn(graph, timestamp) {
  ...
  addRevisions(revs) {
    ...
    for (let i=index, len=this.dataset.length; i<len; ++i) {
      ...
      this.maxVolume.weekly = Math.max(this.maxVolume.weekly, this.incrementColumn(this.el.graphWeekly, weekStamp));
      this.maxVolume.monthly = Math.max(this.maxVolume.monthly, this.incrementColumn(this.el.graphMonthly, monthStamp));
    }
